...
function isLeapYear(date) {
  ...
  return (year % 100 !== 0 && year % 4 === 0) || (year % 4 === 0 && year % 400 === 0);
}
...
function timeSpanToString(startDate, endDate) {
  const milsec = endDate.valueOf() - startDate.valueOf();
  const sss = milsec % 1000;
  const seconds = (milsec - sss) / 1000;
  const ss = seconds % 60;
  const minutes = (seconds - ss) / 60;
  const mm = minutes % 60;
  const hh = (minutes - mm) / 60;

  // eslint-disable-next-line prefer-template, no-nested-ternary
  return `${hh > 9 ? hh : '0' + hh}:${mm > 9 ? mm : '0' + mm}:${ss > 9 ? ss : '0' + ss}.${sss > 99 ? sss : sss > 9 ? '0' + sss : '00' + sss}`;
}
